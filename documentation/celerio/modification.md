...
`${celerio-maven-plugin.outputDir}/src/main/generated-java` or `${celerio-maven-plugin.outputDir}/src/main/java`
...
`${celerio-maven-plugin.outputDir}/src/main/java/<package path>/BankAccount.java` to
...
the `${celerio-maven-plugin.outputDir}` to the `${baseDir}`. In this process you should preserve the file's relative path.
